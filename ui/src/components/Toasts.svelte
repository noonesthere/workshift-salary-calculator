<script lang="ts">
  ...
  import { hideToast, toastStore } from "../stores/toasts";
  import { fly } from "svelte/transition";
  import { ToastType } from "../stores/toasts";
</script>

<div
  class="fixed z-50 inset-0 flex items-start pointer-events-none p-6"
  aria-live="assertive"
>
  <div class="w-full flex flex-col items-end space-y-4">
    {#each $toastStore as toast}
      <div
        class="toast max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black/5 overflow-hidden"
